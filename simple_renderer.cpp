...
///
bool simple_renderer::disable(cgv::render::context &ctx) {
    return surface_renderer::disable(ctx);
}

bool simple_render_style_reflect::self_reflect(cgv::reflect::reflection_handler &rh) {
    return rh.reflect_base(*static_cast<simple_render_style *>(this)) && rh.reflect_member("radius", radius);
...
void simple_renderer::draw(cgv::render::context &ctx, size_t start, size_t count, bool use_strips, bool use_adjacency,
    ...
    draw_impl(ctx, cgv::render::PT_LINES, start, count, use_strips, use_adjacency, strip_restart_index);
}
